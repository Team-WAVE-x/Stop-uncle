...
let coll = db.collection('tokens')
  let allTokens = coll.get().then(snapshot => {
    snapshot.forEach(doc => {
       ...
       db.collection('tokens').doc(temp_token).update({ 'call_count':0 }) 
    })
...
app.use(function (req, res) {
  ...
  db.collection('tokens').doc(req_token).get().then((database) =>{
    if(!database.exists){
      if (quiz === 'exonbabo') {
          return res.send('와 엑슨바보 아시는구나').end()
      } else {
        return res.status(401).send({ message: "Unknown token" }).end()
      }
    } else if (database.data().is_banned){
     return res.status(403).send({ message: "banned token" }).end()
    } else{
      if(database.data().call_count >= max){
        res.status(202).send({ message: "daily limit exceeded" }).end()
      }else{
        db.collection('tokens').doc(req_token).update({ 'call_count':database.data().call_count + 1 }) 
